//! Rust CLI distribution build target.
...
#[cfg(test)]
#[path = "../tests/targets/cli_tests.rs"]
mod cli_tests;
...
pub fn build(sh: &Shell, ctx: &BuildContext, backend: Option<&Backend>) -> Result<()> {
    ...
    output::phase("Rust CLI distribution");
    output::detail("Backend request", backend_label(backend));

    let dist_dir = ctx.cli_artifacts_dir();
    ...
    output::success(format!(
        "CLI distribution build complete in {}",
...
fn prepare_dist_dir(sh: &Shell, dist_dir: &Path) -> Result<()> {
    if dist_dir.exists() {
        output::step(format!(
            "Removing stale CLI artifact directory {}",
            dist_dir.display()
        ));
...
fn build_backend_variant(
    ...
    if matches!(backend, Backend::All) {
        anyhow::bail!("Backend::All cannot be built as a single CLI variant");
    }

    let feature = backend.as_str();
    output::phase(&format!("CLI backend: {}", feature.to_uppercase()));

    let target_dir = ctx.cargo_cli_target_dir(backend);
    let cmake_dir = ctx.cmake_cli_sys_dir(backend);
    ...
    if copy_executable {
        let executable = copy_cli_executable(sh, &target_dir, dist_dir)?;
    ...
    if copy_executable && summary.base_files == 0 {
        anyhow::bail!(
            "CLI {feature} backend did not produce base runtime libraries in {}",
            cmake_dir.display()
        );
    }
    if summary.plugin_files == 0 {
        anyhow::bail!(
            "CLI {feature} backend did not produce ggml backend plugins in {}",
            cmake_dir.display()
        );
    }
...
fn copy_cli_executable(sh: &Shell, target_dir: &Path, dist_dir: &Path) -> Result<PathBuf> {
    let source = target_dir.join("release").join(cli_binary_file_name());
    if !source.exists() {
        anyhow::bail!("cargo did not produce {}", source.display());
    }

    let dest = dist_dir.join(cli_binary_file_name());
