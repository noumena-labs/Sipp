...
fn build_context_paths_are_rooted_under_fake_workspace() {
    ...
    assert_eq!(
        ...
        temp.join(".build/artifacts/npm/sipp/dist/wasm")
    );
    assert_eq!(
        ctx.demo_artifacts_dir("examples"),
    ...
    assert_eq!(
        ctx.tool_artifacts_dir("playground"),
        temp.join(".build/artifacts/tools/playground")
    );
