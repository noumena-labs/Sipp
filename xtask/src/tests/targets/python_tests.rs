...
use super::{
    backend_distribution_name, backend_label, backend_package_project_dir, backends_to_build,
    find_wheel_artifact_for_distribution, prepare_dist_dir, python_project_dir,
...
fn wheel_discovery_matches_normalized_distribution_names() {
    ...
    let sipp = temp.write("dist/sipp-0.1.0-py3-none-any.whl", "");
    let cuda = temp.write("dist/sipp_backend_cuda-0.1.0-cp39-abi3-win_amd64.whl", "");
    temp.write("dist/readme.txt", "");

    assert_eq!(
        find_wheel_artifact_for_distribution(&temp.join("dist"), "sipp").unwrap(),
        Some(sipp)
    );
    assert_eq!(
        find_wheel_artifact_for_distribution(&temp.join("dist"), "sipp-backend-cuda").unwrap(),
        Some(cuda.clone())
    );
    assert!(wheel_matches_distribution(&cuda, "sipp-backend-cuda"));
    assert!(!wheel_matches_distribution(&cuda, "sipp-backend-vulkan"));
}
