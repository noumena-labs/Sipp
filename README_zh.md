...
| 运行环境 | 模块 | 安装 | 文档 |
| --- | --- | --- | --- |
| **Browser** | Sipp Edge | `npm install @sipp/sipp` | [浏览器包](docs/zh/packages/browser.md) |
| **Node.js** | Sipp Core | `npm install @sipp/sipp-server` | [Node.js 包](docs/zh/packages/node.md) |
...
云网关客户端使用完全相同的 `Client` API。网关负责管理模型路径、服务商凭证、访问策略和集中式指标追踪；你的客户端应用代码只需要网关路由目标 URL。
