...
`crates/client` owns local, provider, and gateway endpoint registration through `CogentClient.add(...)`. Gateway endpoints call an HTTP gateway as a client transport and are never selected implicitly.
