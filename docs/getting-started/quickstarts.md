...
instruct GGUFs, render the model's template yourself. `chat` sends role-tagged
...
Use direct provider endpoints only in trusted server code. Provider support is
