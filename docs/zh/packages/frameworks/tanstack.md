...
- 借助 TanStack Start 服务器路由，让浏览器端将该路由作为网关端点调用。
...
服务端函数运行在服务器上，可通过加载器、组件、Hook 或其他服务端函数调用。确保 `@sipphq/sipp-server` 库的导入、提供商密钥的获取和网关 Token 的管理仅局限于服务端函数内部。
...
服务端函数非常适合为应用提供强类型 RPC 接口，返回自定义结构（如 `{ text }`）。但不适用于作为浏览器网关端点调用的代理路由，因为该方法要求端点严格兼容官方网关的 HTTP Profile 格式。
...
浏览器端执行的组件逻辑都必须引入浏览器版本的 `@sipphq/sipp` 包。无论是启动浏览器本地 GGUF 推理，还是利用短期 Token 和同源代理路由发起网关调用，都遵循这一原则。
...
切勿在任何面向浏览器的模块中导入 Node 专用的 `@sipphq/sipp-server`。
