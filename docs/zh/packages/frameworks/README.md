...
| 网关 Profile 路由 | `@sipphq/sipp-server` | 为浏览器网关端点提供兼容代理路由。 |
...
将框架路由封装成兼容浏览器网关调用的端点时，使用 `@sipphq/sipp-server` 提供的 `decodeGatewayQueryBody()`、`decodeGatewayChatBody()`、`decodeGatewayEmbedBody()` 等解析函数及响应助手函数。这些助手帮助路由专注于鉴权策略、目标转发、提供商映射和客户端生命周期，无需关注底层网关 Profile JSON 编码细节。
