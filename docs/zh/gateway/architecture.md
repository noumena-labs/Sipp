...
`sipp::gateway_core`（`sipp` crate 的 `gateway_core` 模块，由 `gateway` feature 启用）只暴露强类型的 query、chat、embed 执行逻辑：
...
`sipp` 客户端 API 负责注册本地、服务商和网关三种端点。网关端点通过 HTTP 调用远程网关，必须显式指定，不会自动使用。
...
- `GatewayCodec`：官方 Sipp JSON/SSE 协议实现。
...
Rust、Node、Python、浏览器包通过相同的 `.add` 暴露网关端点配置，与本地端点和提供商端点结构一致：
