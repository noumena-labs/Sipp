...
| Metal | 支持 | `metal` | 否 | macOS | Apple Silicon 和 AMD GPU；Intel 集成显卡建议使用 CPU |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Chrome (Windows) | ✅ 已测试 149.0.7827.200 | 57 | 92⁴ | 113 | 113 | 86 | 4 |
| Chrome (macOS) | ✅ 已测试 149.0.7827.199 | 57 | 92⁴ | 113 | 113 | 86 | 4 |
| Edge (Windows) | ✅ 已测试 149.0.4022.80 | 79⁵ | 92⁴ | 113 | 113 | 86 | 79⁵ |
| Firefox (Windows) | 🟡 CPU 已测试 152.0.2 | 52 | 79⁴ | 141 | 141 | 111 | 3.5 |
| Firefox (macOS) | 🟡 CPU 已测试 152.0.3 | 52 | 79⁴ | 145⁶ | 145⁶ | 111 | 3.5 |
| Firefox (Linux) | ❌ 未测试 | 52 | 79⁴ | ⚠ Nightly | ⚠ Nightly | 111 | 3.5 |
| Safari (macOS) | ✅ 已测试 26.4（CPU）· STP（WebGPU） | 11 | 15.2⁴ | 26 | 26 | 16.4 | 4 |
...
Sipp 默认提供 pthread WASM 运行时产物：

| 产物 | 后端/运行时 | 线程数 | Token 流式传输 | 要求 |
| --- | --- | --- | --- | --- |
| `sipp-wasm-pthread.js` | WebGPU + JSPI | 最多 4 个⁷ | `SharedArrayBuffer` 环形缓冲区 | COOP + COEP 响应头、安全上下文 |
| `sipp-wasm-pthread-cpu-nojspi.js` | 纯 CPU，无 JSPI | 最多 4 个⁷ | `SharedArrayBuffer` 环形缓冲区 | COOP + COEP 响应头、安全上下文 |
...
macOS 源码构建可以通过 LunarG SDK 编译 Vulkan，但 LunarG 的 macOS Vulkan
驱动本质上会把 Vulkan 翻译到 Metal。Sipp 不发布 macOS Vulkan 包，因为常规
macOS 使用原生 Metal 后端更简单，而 macOS Vulkan 还会增加 Vulkan loader/ICD
运行时依赖。
...
Metal 仅限 macOS，Docker 容器内不可用。Intel 集成显卡虽然暴露 Metal，
但 Sipp 不把它们作为推荐的 Metal 目标；除非你已经在具体模型、上下文长度和
设备上测试过，并确认 Metal 稳定且快于 CPU，否则这些 Mac 建议使用 CPU 后端。

Apple Silicon 可以通过 Rosetta 2 运行 x64 进程。`darwin-x64` 的 Node 或
Python 原生包只会被 x64 Node/Python 进程使用；原生 arm64 Node/Python
安装会使用 `darwin-arm64` 包，这也是 Apple Silicon 上推荐的路径。
