...
```rust
use sipp::{
    endpoint, LocalTextOptions, SippClient, SippQueryRequest,
    SippTextOptions,
};
use sipp::engine::{
...
async fn run(
    ...
    let endpoint = client.add("default", local).await?;
...
Register endpoints with `add(id, endpoint)`, keep the returned `EndpointRef`,
