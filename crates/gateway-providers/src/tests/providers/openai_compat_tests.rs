...
//     let body = openai_chat_body(&request, ProviderKind::Proxy).expect("chat body");
//     let stream_body = openai_stream_chat_body(&request, ProviderKind::Proxy).expect("stream body");
...
//     let body = openai_embedding_body(&request, ProviderKind::Proxy).expect("embedding body");
...
//     let err =
//         openai_embedding_body(&invalid, ProviderKind::Proxy).expect_err("blank model should fail");
//     assert_eq!(err.kind, ProviderErrorKind::InvalidRequest);

//     let mut invalid = request.clone();
//     invalid.input = " ".to_string();
//     let err =
//         openai_embedding_body(&invalid, ProviderKind::Proxy).expect_err("blank input should fail");
//     assert_eq!(err.kind, ProviderErrorKind::InvalidRequest);
...
//     let err = openai_embedding_body(&invalid, ProviderKind::Proxy)
...
//     let err = openai_models_from_body(&serde_json::json!({ "data": null }), ProviderKind::Proxy)
...
//     });
//     let response =
//         openai_embedding_response_from_body(Some("req-1".to_string()), body, ProviderKind::Proxy)
...
//     ] {
//         let err = openai_embedding_response_from_body(None, body, ProviderKind::Proxy)
