...
use cogentlm_gateway_providers::{
    ...
    OpenAiCompatibleAdapterConfig, OpenAiCompatibleProtocol, ProviderAuth, ProviderChatRequest,
    ProviderGenerationOptions, ProviderOptions, ProviderStreamEvent, SecretString,
};
...
struct Args {
    ...
    /// Extra static header for OpenAI-compatible providers, formatted as name=value. Repeatable.
    #[arg(long = "header")]
    headers: Vec<String>,
}
...
impl ProviderPreset {
    ...
    fn default_api_key_env(self) -> &'static str {
        match self {
            ...
            Self::OpenAiCompatible => "PROVIDER_API_KEY",
        }
    }
...
fn build_transport(
...
) -> Result<GatewayAdapterTransport, Box<dyn std::error::Error>> {
    ...
    match provider {
        ProviderPreset::Openai => Ok(GatewayAdapterTransport::openai(OpenAiAdapterConfig {
        ...
        ProviderPreset::Anthropic => Ok(GatewayAdapterTransport::anthropic(
        ...
        | ProviderPreset::OpenAiCompatible => {
            let base_url = args
                ...
                .ok_or_else(|| {
                    "--base-url is required for --provider open-ai-compatible".to_string()
                })?;

            Ok(GatewayAdapterTransport::openai_compatible(
