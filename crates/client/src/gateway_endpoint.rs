...
use cogentlm_core::{ChatRole, FinishReason, TokenBatch, TokenEmissionStats, TokenUsage};
...
use crate::gateway::{GatewayAuthentication, GatewayEndpointConfig, GatewaySecret};
use crate::io_executor::IoExecutor;
use crate::{
    validate, CogentChatRequest, CogentEmbedRequest, CogentEmbeddingResponse, CogentEmbeddingRun,
    CogentError, CogentQueryRequest, CogentRequestContext, CogentResponseMetadata, CogentResult,
    CogentTextResponse, CogentTextRun, CogentTokenBatches, EndpointCapabilities, EndpointError,
    EndpointRef,
};
...
impl InferenceEndpoint for GatewayEndpoint {
    ...
    fn embed_with_context(
        ...
        let body = match embed_value(&self.target, request, &self.protocol_options) {
            Ok(body) => body,
            Err(error) => return CogentEmbeddingRun::ready_err(error),
        };
        ...
        CogentEmbeddingRun::new(Box::pin(GatewayResponseFuture::new(join, executor)))
    }
}
...
impl GatewayTransport {
    fn new(
        ...
        timeouts: crate::GatewayTimeoutPolicy,
    ) -> CogentResult<Self> {
    ...
    async fn response_error(&self, response: reqwest::Response) -> CogentError {
        ...
        CogentError::Endpoint(error)
    }

    fn transport_error(&self, error: reqwest::Error) -> CogentError {
        endpoint_error(
            if error.is_timeout() {
                "timeout"
            } else {
...
impl SseParser {
    fn push(&mut self, bytes: &[u8]) -> CogentResult<Vec<SseEvent>> {
        self.buffer.extend_from_slice(bytes);
        self.take_events(false)
    }

    fn finish(&mut self) -> CogentResult<Vec<SseEvent>> {
        self.take_events(true)
    }

    fn take_events(&mut self, finish: bool) -> CogentResult<Vec<SseEvent>> {
...
fn parse_event(raw: &[u8]) -> CogentResult<Option<SseEvent>> {
    let raw =
...
fn decode_embedding_response(
    ...
    request_id: Option<String>,
    body: serde_json::Value,
) -> CogentResult<CogentEmbeddingResponse> {
    let values = body
        .get("embedding")
...
fn required_string(body: &serde_json::Value, field: &'static str) -> CogentResult<String> {
    body.get(field)
        .and_then(serde_json::Value::as_str)
        .map(str::to_string)
...
fn optional_usage(body: &serde_json::Value) -> CogentResult<Option<TokenUsage>> {
...
fn decode_usage(value: serde_json::Value) -> CogentResult<TokenUsage> {
...
fn validate_name(value: &str, field: &str) -> CogentResult<()> {
...
fn validate_routes(routes: &crate::GatewayRoutes) -> CogentResult<()> {
    for (name, route) in [
...
fn endpoint_error(kind: impl Into<String>, message: impl Into<String>) -> CogentError {
    CogentError::Endpoint(EndpointError::new(kind, message))
}
