...
fn clean_targets_include_generated_dirs_and_optional_purge_roots() {
    ...
    temp.create_dir("apps/examples");
    temp.create_dir("packages/npm");
    ...
    assert!(base.contains(&ctx.app_dir("examples").join("dist")));
    assert!(base.contains(&ctx.npm_package_dir().join(".turbo")));
    ...
    assert!(expanded.contains(&ctx.app_dir("examples").join("node_modules")));
