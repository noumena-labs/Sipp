...
fn node_workspace_status_checks_every_workspace_node_modules_root() {
    ...
    temp.create_dir("apps/examples/node_modules");
    temp.create_dir("packages/npm/node_modules");
    ...
    assert!(roots.contains(&temp.join("apps/examples/node_modules")));
    assert!(roots.contains(&temp.join("packages/npm/node_modules")));
}
