//! Tests the `lifecycle::types::error` module in `cogentlm-engine`.
//!
...
fn crate_error_conversion_preserves_unsupported_operation() {
    let error = ModelError::from(crate::Error::UnsupportedOperation {
        operation: "chat",
...
fn gguf_error_conversion_maps_each_variant_to_model_error() {
    ...
    assert!(matches!(
        ModelError::from(cogentlm_shard::GgufError::Io(io_error)),
        ModelError::Io(error) if error.kind() == std::io::ErrorKind::NotFound
    ));

    assert!(matches!(
        ModelError::from(cogentlm_shard::GgufError::Invalid("bad metadata".to_string())),
        ModelError::InvalidGgufMetadata(message) if message == "bad metadata"
    ));
    assert!(matches!(
        ModelError::from(cogentlm_shard::GgufError::UnsupportedVersion(1)),
        ModelError::UnsupportedGgufVersion(1)
    ));
    assert!(matches!(
        ModelError::from(cogentlm_shard::GgufError::MetadataTooLarge { max_bytes: 4096 }),
        ModelError::GgufMetadataTooLarge { max_bytes: 4096 }
    ));
    assert!(matches!(
        ModelError::from(cogentlm_shard::GgufError::AlreadySplit(3)),
...
fn crate_error_conversion_wraps_other_runtime_errors() {
    let error = ModelError::from(crate::Error::RuntimeCommand("native failed".to_string()));
