...
struct sipp_chat_templates {
    common_chat_templates_ptr inner;
};

struct sipp_common_params {
    common_params inner;
};

struct sipp_common_init {
    common_params params;
    common_init_result_ptr inner;
};

struct sipp_common_sampler {
    common_params_sampling sampling;
    common_sampler * inner = nullptr;
};

struct sipp_common_checkpoint {
...
sipp_common_params * sipp_common_params_parse_server(
    ...
    try {
        ...
        auto * out = new sipp_common_params();
...
llama_model * sipp_common_init_model(sipp_common_init * init) {
    return init != nullptr && init->inner ? init->inner->model() : nullptr;
}

llama_context * sipp_common_init_context(sipp_common_init * init) {
    return init != nullptr && init->inner ? init->inner->context() : nullptr;
}

const llama_vocab * sipp_common_init_vocab(sipp_common_init * init) {
    llama_model * model = sipp_common_init_model(init);
...
sipp_common_sampler * sipp_common_sampler_init_from_json(
    ...
    try {
        auto * out = new sipp_common_sampler();
...
llama_sampler * sipp_common_sampler_raw(sipp_common_sampler * sampler) {
...
sipp_common_checkpoint * sipp_common_checkpoint_capture(
    ...
    try {
        ...
        auto * checkpoint = new sipp_common_checkpoint();
...
sipp_chat_templates * sipp_chat_templates_init(
    ...
    try {
        auto * out = new sipp_chat_templates();
...
int32_t sipp_llama_decode(llama_context * context, const llama_batch * batch) {
...
int32_t sipp_llama_encode(llama_context * context, const llama_batch * batch) {
    if (context == nullptr || batch == nullptr) {
        return -1;
    }
...
const float * sipp_llama_embeddings_seq(llama_context * context, int32_t seq_id) {
    if (context == nullptr || seq_id < 0) {
        return nullptr;
    }
...
const float * sipp_llama_embeddings_ith(llama_context * context, int32_t i) {
    if (context == nullptr) {
        return nullptr;
    }
...
bool sipp_llama_synchronize(llama_context * context) {
    if (context == nullptr) {
        return false;
    }
...
bool sipp_llama_sampler_accept(llama_sampler * sampler, int32_t token) {
...
sipp_mtmd_context * sipp_mtmd_init_from_file(
    ...
    auto * context = new sipp_mtmd_context();
    context->inner = inner;
    return context;
}
...
sipp_mtmd_input_chunks * sipp_mtmd_input_chunks_init(void) {
    ...
    auto * chunks = new sipp_mtmd_input_chunks();
    chunks->inner = inner;
    return chunks;
}
