...
struct MtmdBitmapDeleter {
  void operator()(cogent_mtmd_bitmap * bitmap) const {
    cogent_mtmd_bitmap_free(bitmap);
  }
};

struct MtmdChunksDeleter {
  void operator()(cogent_mtmd_input_chunks * chunks) const {
    cogent_mtmd_input_chunks_free(chunks);
  }
};

using OwnedMtmdBitmap = std::unique_ptr<cogent_mtmd_bitmap, MtmdBitmapDeleter>;
using OwnedMtmdChunks = std::unique_ptr<cogent_mtmd_input_chunks, MtmdChunksDeleter>;
...
struct NativeRuntime::Impl {
  cogent_common_init * init = nullptr;
  cogent_chat_templates * chat_templates = nullptr;
  cogent_mtmd_context * mtmd = nullptr;

  ~Impl() {
    if (mtmd != nullptr) {
      cogent_mtmd_free(mtmd);
    }
    if (chat_templates != nullptr) {
      cogent_chat_templates_free(chat_templates);
    }
    if (init != nullptr) {
      cogent_common_init_free(init);
    }
  }

  llama_model * model() const {
    return cogent_common_init_model(init);
  }

  llama_context * context() const {
    return cogent_common_init_context(init);
  }

  const llama_vocab * vocab() const {
    return cogent_common_init_vocab(init);
  }
};
...
struct CommonSampler::Impl {
  cogent_common_sampler * sampler = nullptr;

  ~Impl() {
    if (sampler != nullptr) {
      cogent_common_sampler_free(sampler);
    }
  }
};
...
void set_llama_log_quiet(bool quiet) {
  cogent_set_llama_log_quiet(quiet);
}

rust::String backend_observability_json(bool include_details) {
  return take_owned_string(cogent_backend_observability_json(include_details), "{}");
}

rust::String mtmd_default_marker() {
  const char * marker = cogent_mtmd_default_marker();
...
std::unique_ptr<NativeRuntime> load_native_runtime(rust::Str model_path, rust::Str args_json) {
  ...
  char * error = nullptr;
  cogent_common_params * params = cogent_common_params_parse_server(
  ...
  cogent_common_init * init = cogent_common_init_from_params(params, &error);
  cogent_common_params_free(params);
  if (init == nullptr) {
  ...
  impl->init = init;
  impl->chat_templates = cogent_chat_templates_init(cogent_common_init_model(init), "");
...
std::int32_t NativeRuntime::n_ctx() const {
  return cogent_common_init_n_ctx(impl_->init);
}

std::int32_t NativeRuntime::n_batch() const {
  return cogent_common_init_n_batch(impl_->init);
}

std::int32_t NativeRuntime::n_ubatch() const {
  return cogent_common_init_n_ubatch(impl_->init);
}

std::int32_t NativeRuntime::n_seq_max() const {
  return cogent_common_init_n_parallel(impl_->init);
}
...
std::int32_t NativeRuntime::n_embd_out() const {
  return cogent_common_init_n_embd_out(impl_->init);
}

std::int32_t NativeRuntime::n_cls_out() const {
  return cogent_common_init_n_cls_out(impl_->init);
}

std::int32_t NativeRuntime::pooling_type() const {
  return cogent_common_init_pooling_type(impl_->init);
}

bool NativeRuntime::has_encoder() const {
  return cogent_common_init_model_has_encoder(impl_->init);
}

bool NativeRuntime::has_decoder() const {
  return cogent_common_init_model_has_decoder(impl_->init);
}

bool NativeRuntime::has_chat_template() const {
  return cogent_common_init_model_has_chat_template(impl_->init);
}
...
bool NativeRuntime::kv_unified() const {
  return cogent_common_init_kv_unified(impl_->init);
}

rust::String NativeRuntime::flash_attention() const {
  return take_owned_string(cogent_common_init_flash_attention(impl_->init), "unknown");
}

rust::String NativeRuntime::cache_type_k() const {
  return take_owned_string(cogent_common_init_cache_type_k(impl_->init), "unknown");
}

rust::String NativeRuntime::cache_type_v() const {
  return take_owned_string(cogent_common_init_cache_type_v(impl_->init), "unknown");
}

rust::String NativeRuntime::chat_template_source() const {
  return take_owned_string(cogent_chat_templates_source(impl_->chat_templates), "");
}
...
std::int32_t NativeRuntime::decoder_start_token() const {
  return cogent_common_init_decoder_start_token(impl_->init);
}
...
rust::String NativeRuntime::apply_chat_template_json(
  ...
  char * rendered = cogent_apply_chat_template(
...
std::int32_t NativeRuntime::decode(const NativeBatch & batch) {
  const std::int32_t status = cogent_llama_decode(impl_->context(), &batch.impl_->batch);
  if (status < 0) {
...
std::int32_t NativeRuntime::encode(const NativeBatch & batch) {
  const std::int32_t status = cogent_llama_encode(impl_->context(), &batch.impl_->batch);
  if (status < 0) {
...
bool NativeRuntime::synchronize() {
  return cogent_llama_synchronize(impl_->context());
}
...
rust::Vec<float> NativeRuntime::embeddings_seq(std::int32_t seq_id) const {
  const float * values = cogent_llama_embeddings_seq(impl_->context(), seq_id);
  if (values == nullptr) {
    throw std::runtime_error("llama embeddings sequence output is unavailable");
  }

  std::int32_t dimensions = cogent_common_init_n_embd_out(impl_->init);
  if (cogent_common_init_pooling_type(impl_->init) == LLAMA_POOLING_TYPE_RANK) {
    dimensions = cogent_common_init_n_cls_out(impl_->init);
  }
  if (dimensions <= 0) {
...
rust::Vec<std::uint8_t> NativeRuntime::state_seq(std::int32_t seq_id) const {
  ...
  std::size_t size = 0;
  const bool ok = cogent_llama_state_seq_get_data_ext_alloc(
      impl_->context(),
      seq_id,
      kStateSeqFlagsNone,
      &data,
...
bool NativeRuntime::set_state_seq(std::int32_t seq_id, rust::Slice<const std::uint8_t> data) {
  return cogent_llama_state_seq_set_data_ext(
      impl_->context(),
      seq_id,
      kStateSeqFlagsNone,
      data.data(),
...
bool NativeRuntime::init_mtmd(rust::Str projector_path, bool use_gpu, std::int32_t n_threads) {
  ...
  if (impl_->mtmd != nullptr) {
    cogent_mtmd_free(impl_->mtmd);
    impl_->mtmd = nullptr;
  }
  impl_->mtmd = cogent_mtmd_init_from_file(path.c_str(), impl_->model(), use_gpu, n_threads);
  return impl_->mtmd != nullptr;
}

bool NativeRuntime::mtmd_support_vision() const {
  return impl_->mtmd != nullptr && cogent_mtmd_support_vision(impl_->mtmd);
}
...
std::int32_t NativeRuntime::mtmd_eval_images(
  ...
  std::vector<const cogent_mtmd_bitmap *> bitmap_refs;
  ...
  for (std::size_t i = 0; i < image_sizes.size(); ++i) {
    const auto len = static_cast<std::size_t>(image_sizes[i]);
    cogent_mtmd_bitmap * bitmap =
        cogent_mtmd_bitmap_init_from_buf(impl_->mtmd, image_bytes.data() + offset, len);
    if (bitmap == nullptr) {
  ...
  OwnedMtmdChunks chunks(cogent_mtmd_input_chunks_init());
  if (chunks == nullptr) {
  ...
  const bool tokenized = cogent_mtmd_tokenize(
      impl_->mtmd,
      chunks.get(),
      text.c_str(),
      add_special,
  ...
  const std::int32_t status = cogent_mtmd_eval_chunks(
      impl_->mtmd,
      impl_->context(),
      chunks.get(),
      n_past,
  ...
  if (!cogent_llama_synchronize(impl_->context())) {
...
std::unique_ptr<CommonSampler> create_sampler(
  ...
  char * error = nullptr;
  cogent_common_sampler * sampler = cogent_common_sampler_init_from_json(
      runtime.impl_->init,
      sampling.c_str(),
      grammar_text.c_str(),
      schema_text.c_str(),
...
bool CommonSampler::sampler_accept(std::int32_t token, bool accept_grammar) {
  return cogent_common_sampler_accept(impl_->sampler, token, accept_grammar);
}
...
bool CommonSampler::sampler_backend_sampling() const {
  return cogent_common_sampler_backend_sampling(impl_->sampler);
}

std::int32_t sampler_sample(CommonSampler & sampler, NativeRuntime & runtime, std::int32_t idx) {
  return cogent_common_sampler_sample(sampler.impl_->sampler, runtime.impl_->context(), idx);
}

bool sampler_attach(CommonSampler & sampler, NativeRuntime & runtime, std::int32_t seq_id) {
  return cogent_llama_set_sampler(
      runtime.impl_->context(),
      seq_id,
      cogent_common_sampler_raw(sampler.impl_->sampler));
}

bool sampler_detach(NativeRuntime & runtime, std::int32_t seq_id) {
  return cogent_llama_set_sampler(runtime.impl_->context(), seq_id, nullptr);
}
