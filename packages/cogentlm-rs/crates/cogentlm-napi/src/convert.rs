...
pub(super) fn core_error(error: cogentlm_engine::Error) -> Error {
    match error {
...
pub(super) fn model_error(error: cogentlm_engine::lifecycle::ModelError) -> Error {
    match error {
        cogentlm_engine::lifecycle::ModelError::InvalidModelSource(message)
        | cogentlm_engine::lifecycle::ModelError::InvalidModelPairing(message) => invalid_arg(message),
