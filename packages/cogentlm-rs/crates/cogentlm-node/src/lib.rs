...
use cogentlm_core::{
    ...
    set_llama_log_quiet as core_set_llama_log_quiet, CacheKeyPolicy,
    ChatMessage as CoreChatMessage, ChatRequest as CoreChatRequest, ChatRole as CoreChatRole,
    ...
    LoadedModelInfo as CoreLoadedModelInfo, LogitBias, ModelInfo as CoreManagedModelInfo,
    ModelLoadOptions as CoreModelLoadOptions, ModelModality as CoreModelModality,
    ModelPlacementConfig as CoreModelPlacementConfig, ModelService as CoreModelService,
    ModelServiceState as CoreModelServiceState, ModelSourceKind as CoreModelSourceKind,
    ModelStatus as CoreModelStatus, MultimodalRuntimeConfig as CoreMultimodalRuntimeConfig,
    ...
    SchedulerRuntimeConfig as CoreSchedulerRuntimeConfig, SplitMode, StatsMode,
    TokenBatch as CoreTokenBatch,
};
use cogentlm_core::{
...
impl ContextRuntimeConfig {
    fn to_core(&self) -> Result<cogentlm_core::ContextRuntimeConfig> {
        let mut core = cogentlm_core::ContextRuntimeConfig {
            n_ctx: self.n_ctx,
...
impl CacheRuntimeConfig {
    fn to_core(&self) -> Result<cogentlm_core::CacheRuntimeConfig> {
...
fn with_engine<T>(
    engine: &SharedEngine,
    f: impl FnOnce(&CoreCogentEngine) -> cogentlm_core::Result<T>,
) -> Result<T> {
    let guard = engine
        .lock()
        .map_err(|_| napi_error("engine mutex is poisoned"))?;
...
fn with_model_service<T>(
    service: &SharedModelService,
    f: impl FnOnce(&CoreModelService) -> std::result::Result<T, cogentlm_core::ModelError>,
) -> Result<T> {
    let guard = service
        .lock()
        .map_err(|_| napi_error("model service mutex is poisoned"))?;
...
fn with_model_service_mut<T>(
    service: &SharedModelService,
    f: impl FnOnce(&mut CoreModelService) -> std::result::Result<T, cogentlm_core::ModelError>,
) -> Result<T> {
...
) -> std::result::Result<(), cogentlm_core::ModelError> {
    let receiver = service.subscribe_events()?;
    events
        .lock()
        .map_err(|_| {
            cogentlm_core::ModelError::Runtime("model service events mutex is poisoned".to_string())
        })?
        .replace(receiver);
    Ok(())
}
...
fn emit_token_batch(
    ...
    if status == Status::Ok {
        ...
        Err(cogentlm_core::Error::RuntimeCommand(format!(
...
fn core_error(error: cogentlm_core::Error) -> Error {
    match error {
...
fn model_error(error: cogentlm_core::ModelError) -> Error {
    match error {
