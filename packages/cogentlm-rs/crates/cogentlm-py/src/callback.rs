...
use cogentlm_engine::engine::{RequestResult, TokenBatch};
...
pub(super) fn to_py_error(error: cogentlm_engine::Error) -> PyErr {
    match error {
...
pub(super) fn to_py_model_error(error: cogentlm_engine::lifecycle::ModelError) -> PyErr {
    match error {
        cogentlm_engine::lifecycle::ModelError::InvalidModelSource(message)
        | cogentlm_engine::lifecycle::ModelError::InvalidModelPairing(message) => PyValueError::new_err(message),
