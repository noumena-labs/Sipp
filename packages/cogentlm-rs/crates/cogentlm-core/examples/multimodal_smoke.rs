...
use cogentlm_core::{
    backend_observability_json, default_media_marker, set_llama_log_quiet,
    vision_model_source_from_paths, BackendPreference, ChatMessage, ChatRequest, EngineEvent,
    EngineEventReceiver, GpuLayerConfig, ModelLoadOptions, ModelService, NativeRuntimeConfig,
    QueryOptions, QueryRequest, RequestResult, SamplingRuntimeConfig, StatsMode,
};
...
impl Args {
    fn parse() -> anyhow::Result<Self> {
        ...
        let Some(model) = args.next() else {
            anyhow::bail!(
                "usage: cargo run -p cogentlm-core --example multimodal_smoke -- <model.gguf> <mmproj.gguf> <image.png> [prompt] [--max-tokens N] [--ctx-size N] [--threads N] [--gpu-layers N] [--backend auto|cpu|cuda|metal|vulkan|webgpu] [--model-store PATH] [--seed N] [--temperature F] [--stats off|basic|profile] [--cpu-mmproj] [--no-marker-in-message] [--raw] [--verbose-llama]"
            );
        };
