...
Node.js 包发布名称为 `sipp-server`。为 Node 服务器进程、路由处理器和框架服务端函数提供 Sipp 客户端 API。使用该包时，应用全权负责管理框架的路由、请求校验、身份验证和部署策略。
...
仅在 Node 运行时代码中使用此包。浏览器组件改用 [`sipp`](browser.md)。
...
只在服务端代码中使用 `sipp-server`。典型场景包括配置了 `runtime = 'nodejs'` 的 Next.js App Router 路由处理器、TanStack Start 服务端函数、Express 路由或后台工作进程。切勿将其引入浏览器包。
