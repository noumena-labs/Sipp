...
Node.js 包发布名称为 `cogentlm-server`。为 Node 服务器进程、路由处理器和框架服务端函数提供 CogentLM 客户端 API。使用该包时，应用全权负责管理框架的路由、请求校验、身份验证和部署策略。
...
仅在 Node 运行时代码中使用此包。浏览器组件改用 [`cogentlm`](browser.md)。
...
设置环境变量 `COGENTLM_NODE_BACKEND=cpu|vulkan|cuda|metal` 来选择原生后端引擎。关于本地运行时的配置参数与请求选项说明，请参阅[运行时选项](../reference/runtime-options.md)。
...
只在服务端代码中使用 `cogentlm-server`。典型场景包括配置了 `runtime = 'nodejs'` 的 Next.js App Router 路由处理器、TanStack Start 服务端函数、Express 路由或后台工作进程。切勿将其引入浏览器包。
