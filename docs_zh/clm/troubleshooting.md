...
`clm` 无法激活时，可直接用 `cargo xtask` 执行相同命令：
...
如果 xtask 源文件、工作区清单或 Cargo 配置的修改时间晚于 `.build/xtask/clm.stamp`，安装脚本会重新构建 `.build/xtask/debug/xtask`。拉取含构建脚本更新的代码后，这是正常行为。
...
注意：CUDA 不由 xtask 自动安装。请从 NVIDIA 官方渠道安装 CUDA，然后重新运行 `clm doctor --target node --backend cuda` 检查。
