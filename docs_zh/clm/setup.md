...
在项目根目录下运行安装脚本。它会在需要时构建 `xtask` 二进制文件，将 `clm` 启动器安装到 `.build/bin`，并为选定的工作流准备工具链和示例文件。
...
PowerShell 脚本会更新当前会话的 `PATH`，安装成功后加载 `.build\bin\cogentlm-env.ps1`。
...
- `.build/bin/clm`、`.build/bin/clm.cmd`、`.build/bin/clm.ps1`
- `.build/bin/cogentlm-env.sh`、`.build/bin/cogentlm-env.ps1`
