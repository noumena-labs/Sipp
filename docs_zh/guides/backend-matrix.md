...
CogentLM 的本地推理构建在 llama.cpp 和 ggml 之上。CogentLM 负责客户端 API、端点模型、调度、各语言包的绑定、浏览器的生命周期管理以及网关集成；llama.cpp 和 ggml 负责提供 GGUF 运行时的底层后端计算内核。

因此后端支持情况分为两层：

- CogentLM 层：各语言包支持哪些后端，以及如何编译或选择这些后端。
...
| `metal` | Apple GPU | macOS 原生构建、Node.js、Python、CLI、网关服务 | 仅限 macOS 原生环境。 |
| `vulkan` | 兼容 Vulkan 的 GPU | 原生构建、Node.js、Python、CLI、网关服务 | 需系统和驱动支持。xtask 会在编译时按需准备 Vulkan SDK。 |

上游 llama.cpp/ggml 还支持 BLAS、CANN、OpenCL、SYCL 等后端（可在运算表中看到），但目前 CogentLM 官方暴露的后端仅为上述五种。
...
| Node.js | `cpu`, `vulkan`, `cuda`, `metal` | `COGENTLM_NODE_BACKEND=cpu|vulkan|cuda|metal` |
| Python | `cpu`, `vulkan`, `cuda`, `metal` | `COGENTLM_PYTHON_BACKEND=cpu|vulkan|cuda|metal` |
| CLI | `auto`, `cpu`, `cuda`, `metal`, `vulkan` | `cogentlm ... --backend <backend>` |
| 网关服务 | `auto`, `cpu`, `cuda`, `metal`, `vulkan` | 通过 `clm ... --backend <backend>` 编译或运行；目标配置 TOML 可以设定 `backend = "auto"` 或特定名称。 |
| Rust / 源码工作流 | 编译后的产物集合 | 通过 `clm` 或 `cargo xtask` 编译；运行时可用后端取决于链接的原生产物。 |

注意：`auto` 是运行时策略。`all` 是 `clm` 和 `xtask` 在构建/测试时使用的参数，表示编译当前主机支持的所有后端，不是运行时的后端名称。
...
- **`cpu`** 是特例。明确指定引擎使用 `cpu` 时，CogentLM 会禁用 GPU 计算、显存分配、KV 缓存卸载、运算卸载、Flash Attention 和 GPU 驻留管理。
- **GPU 引擎**（`cuda`、`metal`、`vulkan`、`webgpu`）必须同时满足两个条件：编译时已包含进产物，且运行时环境可用。
- Node.js 和 Python 在进程启动时通过环境变量（`COGENTLM_NODE_BACKEND` 或 `COGENTLM_PYTHON_BACKEND`）加载特定原生绑定。同一进程内只能使用一种原生后端。如需不同后端，需启动独立进程或加载不同产物。
...
| 构建命令 | 后端参数 | 产出物 |
| --- | --- | --- |
| `clm build wasm` | 无 | 包含 CPU 与 WebGPU 运行时的浏览器 WASM 包。 |
| `clm build node --backend cpu` | `cpu`, `cuda`, `metal`, `vulkan`, `all` | 针对选定后端编译的 Node 原生绑定。 |
| `clm build python --backend cpu` | `cpu`, `cuda`, `metal`, `vulkan`, `all` | 针对选定后端编译的 Python 原生绑定。 |
| `clm build cli --backend cpu` | `cpu`, `cuda`, `metal`, `vulkan`, `all` | 针对选定后端编译的 `cogentlm` CLI 工具。 |
| `clm build gateway-server --backend cpu` | `cpu`, `cuda`, `metal`, `vulkan`, `all` | 针对选定后端编译的网关服务程序。 |
| `clm build all` | 无 | Core, WASM, Python CPU, Node CPU 以及 CLI CPU 目标。 |

`clm build all` 仅生成基础组合。如果需要 CUDA、Metal 或 Vulkan 支持，必须显式传入对应的 `--backend` 参数。

## 运算支持情况

并非所有 ggml 后端都实现了完整的运算集。CogentLM 提供的主流后端已覆盖了常用的 Transformer 架构，但不同模型所需的特殊运算可能会受限于你所选的后端。
...
`llama backend-ops` 会为指定的后端构建 llama.cpp 的测试工具，非常适合在 CogentLM 客户端之外排查运算覆盖率和性能瓶颈。
...
针对原生部署：NVIDIA 硬件环境使用 `cuda`；macOS 环境使用 `metal`；如果需要兼容多厂商的 GPU，并在目标驱动栈上验证通过，则使用 `vulkan`。
