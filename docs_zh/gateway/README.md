...
Sipp 网关在本地 GGUF 和服务商前面架一层统一的 HTTP 边界。应用层代码写法不变：`SippClient.add` 注册端点，获取引用，将引用传入 `query`、`chat`、`embed`进行推理。
...
- **Docker 镜像**：`apps/gateway-server/Dockerfile`，运行 `sipp-gateway serve --config /etc/sipp/gateway.toml`。
