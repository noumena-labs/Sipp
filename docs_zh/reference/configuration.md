...
CogentLM 的配置严格按职责划分。核心 Crate 不处理 HTTP 路由、身份验证方案、TOML 文件或部署策略。
...
- `COGENTLM_GATEWAY_TOKEN`：开发环境或示例中使用的网关 Bearer Token。
- `COGENTLM_GATEWAY_ADMIN_PASSWORD`：网关示例中使用的管理面板密码。
- `COGENTLM_GATEWAY_URL`：客户端示例使用的网关基础 URL。
- `COGENTLM_NODE_BACKEND`：Node 运行时的计算后端选择。
- `COGENTLM_PYTHON_BACKEND`：Python 运行时的计算后端选择。
