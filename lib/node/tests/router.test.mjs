...
test('router imports the selected built binding and exposes backend helpers', () => {
  ...
  assert.equal(typeof binding.EndpointDescriptor.files, 'function');
  assert.equal(typeof binding.EndpointDescriptor.urls, 'function');
...
test('public declarations expose one endpoint descriptor factory', () => {
  ...
  assert.match(
    declarations,
    /const EndpointDescriptor:[\s\S]*files\([\s\S]*urls\([\s\S]*gateway\([\s\S]*provider\(/
  );
...
test('remote 503 errors preserve lifecycle metadata after the shared retry policy', async () => {
  ...
  try {
    ...
    await assert.rejects(
      client.add('remote-model', binding.EndpointDescriptor.urls(
