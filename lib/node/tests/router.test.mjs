...
test('public declarations expose one endpoint factory', () => {
  ...
  assert.match(
    declarations,
    /interface SippListenRequest[\s\S]*maxTokens\?: number/
  );
});
...
test('same-id endpoint replacement is owned by the native client', async () => {
  ...
  try {
    ...
    const response = await client.query({ endpoint: original, prompt: 'route' }).response;
...
test('remote 503 errors preserve lifecycle metadata after the shared retry policy', async () => {
  ...
  try {
    ...
    await assert.rejects(
      client.models.add([new URL(`http://127.0.0.1:${address.port}/model.gguf`)]),
      (error) => {
...
(async () => {
  ...
  const models = new binding.ModelStore();
  assert.deepEqual(
    models.add([new URL('https://models.test/model.gguf')]),
