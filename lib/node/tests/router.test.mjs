...
test('public declarations expose one endpoint factory', () => {
  ...
  assert.match(
    declarations,
    /type ModelLoadMode =[\s\S]*'auto'[\s\S]*'direct_io'/
  );
  assert.doesNotMatch(declarations, /use_mmap|use_mlock/);
  assert.match(
    declarations,
...
test('endpoint factories create opaque native endpoint values', () => {
  ...
  assert.ok(Endpoint.local(model, {
    runtime: { placement: { load_mode: 'direct_io' } },
  }) instanceof Endpoint);
  assert.throws(
    () => Endpoint.local(model, {
      runtime: { placement: { load_mode: 'legacy' } },
    }),
