...
test('public model declarations require one explicit source variant', () => {
  const declarations = readFileSync(path.join(bindingDir, 'index.d.ts'), 'utf8');

  assert.match(declarations, /readonly kind: 'installed'[\s\S]*readonly modelId: string/);
  assert.match(declarations, /readonly kind: 'local'[\s\S]*readonly modelPaths: readonly string\[\]/);
  assert.match(declarations, /readonly kind: 'remote'[\s\S]*readonly modelUrls: readonly string\[\]/);
  assert.match(declarations, /readonly source: ModelSource[\s\S]*readonly storageRoot: string/);
...
test('remote 503 errors preserve lifecycle metadata after the shared retry policy', async () => {
  ...
  try {
    ...
    const client = new binding.SippClient();

    await assert.rejects(
      client.add('remote-model', {
        kind: 'local',
        source: {
          kind: 'remote',
          modelUrls: [`http://127.0.0.1:${address.port}/model.gguf`],
        },
        storageRoot,
      }),
      (error) => {
