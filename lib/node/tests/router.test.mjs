...
test('router imports the selected built binding and exposes backend helpers', () => {
  ...
  assert.equal(typeof binding.LocalEndpointDescriptor.files, 'function');
  assert.equal(typeof binding.LocalEndpointDescriptor.urls, 'function');
  assert.equal(typeof binding.LocalEndpointDescriptor.installed, 'function');
...
test('public model declarations expose only local endpoint factories', () => {
  const declarations = readFileSync(path.join(bindingDir, 'index.d.ts'), 'utf8');

  assert.match(declarations, /const LocalEndpointDescriptor:[\s\S]*files\([\s\S]*urls\([\s\S]*installed\(/);
...
test('remote 503 errors preserve lifecycle metadata after the shared retry policy', async () => {
  ...
  try {
    ...
    await assert.rejects(
      client.add('remote-model', binding.LocalEndpointDescriptor.urls(
