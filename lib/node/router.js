...
function loadCandidate(backend, triplet) {
  ...
  for (const candidate of backendBinaryPaths(backend, triplet)) {
    try {
      const binding = requireBinaryCandidate(candidate)
...
const nativeAddModel = binding.ModelStore.prototype.add
binding.ModelStore.prototype.add = function addModel(sources) {
  return nativeAddModel.call(
    this,
    sources.map((source) => (source instanceof URL ? source.href : source)),
  )
}
...
const EndpointDescriptor = Object.freeze({
  local(modelId, options = {}) {
    return {
      kind: 'local',
      modelId,
  ...
  gateway(options) {
    return {
      ...
      queryRoute: options.queryRoute,
      chatRoute: options.chatRoute,
      embedRoute: options.embedRoute,
...
module.exports.EndpointDescriptor = EndpointDescriptor
