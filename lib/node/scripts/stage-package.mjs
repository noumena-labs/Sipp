...
const stageDir = path.join(repoRoot, '.build', 'artifacts', 'npm', 'sipp-server');
...
const packageFiles = ['gateway-profile.js', 'index.d.ts', 'router.d.ts', 'router.js'];
...
const nativeArtifacts = (await readdir(sourceArtifactDir))
  .filter((fileName) => fileName.startsWith('sipp_node_') && fileName.endsWith('.node'))
  .sort();
