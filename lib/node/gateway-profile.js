...
function decodeGatewayQueryBody(body) {
  ...
  return {
    ...
    input: prompt,
    options: textCallOptions(value, TEXT_TYPED_FIELDS),
  }
}
...
function decodeGatewayChatBody(body) {
  ...
  return {
    target,
    stream: streamFlag(value.stream),
    input: chatMessages(value.messages),
    options: textCallOptions(value, TEXT_TYPED_FIELDS),
  }
}
...
function decodeGatewayEmbedBody(body) {
  ...
  const extra = profileOptions(value, EMBED_TYPED_FIELDS)
  return {
    target,
    stream: false,
    input,
    options: Object.keys(extra).length === 0 ? {} : { extra },
  }
}
...
function textCallOptions(body, typedFields) {
  const options = textOptions(body)
  const extra = profileOptions(body, typedFields)
  return {
